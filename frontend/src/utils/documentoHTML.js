// Arma el documento final en HTML, listo para ver/imprimir/guardar como PDF desde el
// navegador. No depende de una plantilla fija: recibe el texto ya armado (con los
// placeholders ya reemplazados), la tabla de campos personalizados, y hasta 2 firmantes
// (cada uno con su firma digital guardada, si la tiene).
...
// Reemplaza los tokens {{persona}}, {{cargo}}, {{fecha}}, {{firmante2}}, {{firmante2Cargo}}
// en el texto libre de una plantilla, por los datos reales de esta generación.
export function reemplazarTokens(texto, valores) {
  return texto.replace(/\{\{(\w+)\}\}/g, (match, clave) => (valores[clave] !== undefined ? valores[clave] : match));
}

// Convierte el nombre de un campo personalizado en una "clave" simple (sin tildes/espacios)
